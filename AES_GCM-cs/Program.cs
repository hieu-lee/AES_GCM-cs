...
namespace AES_GCM_cs
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            bool MemCmp(byte[] arr1, byte[] arr2, ulong size)
            {
            ...
            for (len_p = 0; len_p <= 3; len_p++)
            {
                ...
                {
                    aes128gcm.AES128GCM(ciphertext, tag, key, IV, plaintext, len_p, add_data, len_ad);

                    Console.WriteLine($"lenghts {len_p} {len_ad}");

                    Console.WriteLine($"ciphertext success: {MemCmp(ciphertext, ciphertext_ref, len_p * 16)}");

                    var x = new byte[16];
                    for (int i = 0; i < 16; i++)
                    {
                        x[i] = tag_ref[len_p * 4 + len_ad, i];
                    }
